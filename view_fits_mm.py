...
def get_smpl(pkl_data):
    ...
    # smpl_o3d_offset.paint_uniform_color([0.7, 0.3, 0.3])

    all_markers = []
    cur_participant_err = list()
    all_joint_err.append(cur_participant_err)
    for i in range(25):
        ...
        cur_joint_err = dict()
        cur_joint_err['gt'] = gt_pos_3d[i, :]
        cur_joint_err['smpl'] = pos
        cur_joint_err['joint'] = i
        cur_participant_err.append(cur_joint_err)

        # print('j {} gt pos {}, smpl pos {}, err {:.3f}'.format(i, gt_pos_3d[i, :], pos, np.linalg.norm(gt_pos_3d[i, :] - pos)))
...
if __name__ == "__main__":
    ...
    pickle.dump(all_joint_err, open('joint_error.pkl', 'wb'))
